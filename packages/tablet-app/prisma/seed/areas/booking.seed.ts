 ...
 * 1. Default appointment types
 * 2. Sample appointments (standard/full mode only)
...
export async function seedBooking(ctx: SeedContext): Promise<void> {
  ...
  if (clinicIds.length === 0) {
    logger.warn('No clinics found, skipping booking seeding');
    logger.endArea('Booking', 0);
    return;
  }
  ...
  for (let clinicIndex = 0; clinicIndex < clinicIds.length; clinicIndex++) {
    ...
    if (!createdBy) {
      logger.warn(`  No admin user found for clinic ${clinicId}, skipping booking seeding for this clinic`);
      continue;
    }
    ...
    // ============================================================================
    logger.info('  Seeding appointment types...');
    let clinicTypesCreated = 0;
    ...
    for (const typeData of DEFAULT_APPOINTMENT_TYPES) {
      ...
      if (!existingType) {
        ...
        idTracker.add('AppointmentType', appointmentType.id, clinicId);
        clinicTypesCreated++;
        totalTypesCreated++;
      } else {
        idTracker.add('AppointmentType', existingType.id, clinicId);
      }
    }

    logger.info(`    Created ${clinicTypesCreated} appointment types`);

    // ============================================================================
    // 2. SEED EMERGENCY PROTOCOLS
    // ============================================================================
    logger.info('  Seeding emergency protocols...');
    ...
    for (const protocolData of DEFAULT_EMERGENCY_PROTOCOLS) {
      ...
      if (!existingProtocol) {
        ...
        idTracker.add('EmergencyProtocol', protocol.id, clinicId);
    ...
    // ============================================================================
    logger.info('  Seeding reminder templates...');
    ...
    for (const templateData of DEFAULT_REMINDER_TEMPLATES) {
      ...
      if (!existingTemplate) {
        ...
        idTracker.add('ReminderTemplate', template.id, clinicId);
        totalEmergencyDataCreated++;
      }
    }
    ...
    // ============================================================================
    // 5. SEED EMERGENCY FAQs
    // ============================================================================
    logger.info('  Seeding emergency FAQs...');
    ...
    for (const faqData of DEFAULT_EMERGENCY_FAQS) {
      ...
      if (!existingFaq) {
        ...
        idTracker.add('EmergencyFAQ', faq.id, clinicId);
        totalEmergencyDataCreated++;
      }
    }
    ...
    if (config.mode === 'minimal') {
      logger.info('  Skipping sample appointments in minimal mode');
      continue;
    }

    logger.info('  Seeding sample appointments...');

    // Get patients from this clinic (using standardized soft delete filter)
    const patients = await db.patient.findMany({
      where: withSoftDelete({ clinicId }),
      take: config.counts.patientsPerClinic,
      select: { id: true },
    });
    ...
    if (patients.length === 0) {
      logger.warn(`    No patients found for clinic ${clinicId}, skipping sample data`);
      continue;
    }

    if (providers.length === 0) {
      logger.warn(`    No providers found for clinic ${clinicId}, skipping sample data`);
      continue;
    }

    if (appointmentTypes.length === 0) {
      logger.warn(`    No appointment types found for clinic ${clinicId}, skipping sample data`);
      continue;
    }
    ...
    const bookedBy = bookingUser?.id || providerIds[0];
    ...
    let clinicAppointmentsCreated = 0;
    ...
    for (const apptData of sampleAppointments) {
      ...
      if (!conflict) {
        ...
        idTracker.add('Appointment', appointment.id, clinicId);
        createdAppointments.push({ id: appointment.id, patientId: apptData.patientId });
        clinicAppointmentsCreated++;
        totalAppointmentsCreated++;
      }
    }

    logger.info(`    Created ${clinicAppointmentsCreated} sample appointments`);

    // ============================================================================
    // 7. SEED SAMPLE EMERGENCY DATA
    // ============================================================================
    ...
    for (const emergencyData of sampleEmergencies) {
      ...
      idTracker.add('EmergencyAppointment', emergency.id, clinicId);
      totalEmergencyDataCreated++;
    }
    ...
    for (const scheduleData of onCallSchedules) {
      ...
      idTracker.add('OnCallSchedule', schedule.id, clinicId);
      totalEmergencyDataCreated++;
    }
    ...
    if (reminderTemplates.length > 0 && createdAppointments.length > 0) {
      ...
      for (const reminderData of sampleReminders) {
        ...
        idTracker.add('AppointmentReminder', reminder.id, clinicId);
        totalEmergencyDataCreated++;
      }
    ...
    // ============================================================================
    // 10. SEED SAMPLE AFTER-HOURS MESSAGES
    // ============================================================================
    ...
    for (const messageData of afterHoursMessages) {
      ...
      idTracker.add('AfterHoursMessage', message.id, clinicId);
      totalEmergencyDataCreated++;
    }
    ...
    for (const entryData of waitlistEntries) {
      ...
      idTracker.add('WaitlistEntry', entry.id, clinicId);
      waitlistCount++;
    }
    ...
    for (let i = 0; i < cancellationData.length; i++) {
      ...
      const actualAppointment = appointmentsForCancellation[i % Math.max(appointmentsForCancellation.length, 1)];

      // Skip if we don't have valid appointment reference
      const appointmentId = actualAppointment?.id || createdAppointments[i % Math.max(createdAppointments.length, 1)]?.id;
      ...
      idTracker.add('AppointmentCancellation', cancellation.id, clinicId);
      cancellationCount++;
    }
    ...
    for (const riskData of riskScoreData) {
      ...
      idTracker.add('PatientRiskScore', riskScore.id, clinicId);
      riskScoreCount++;
    }
    ...
    if (providers.length > 0) {
      ...
      for (const blockData of scheduleBlocksData) {
        ...
        idTracker.add('ScheduleBlock', block.id, clinicId);
        scheduleBlockCount++;
      }
    }
    ...
    for (const templateData of DEFAULT_BOOKING_TEMPLATES) {
      ...
      if (!existingTemplate) {
        ...
        idTracker.add('BookingTemplate', template.id, clinicId);
    ...
    // ============================================================================
    // 17. SEED RECURRING APPOINTMENTS
    // ============================================================================
    ...
    for (const recurring of recurringData) {
      ...
      idTracker.add('RecurringAppointment', recurringAppt.id, clinicId);
  ...
  logger.success(`\nBooking seeding complete: ${totalTypesCreated} types, ${totalAppointmentsCreated} appointments, ${totalEmergencyDataCreated} emergency/reminder records`);
